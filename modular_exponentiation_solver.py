...
def parse(problem statement):
	...
	x_power, quotient, modulus_x = map(int, match.groups())
...
def pick_one_root_power_pair(root_power_pairs):
	...
	min_index = argmin(root_power_pair_results)
...
def downtairing_mod(val, downstairs_modulus_y, upstairs_modulus_y):
    pass
...
def main():
    ...
    # Find the best root, power pair for handsolving (smallest result)
    ...
    inverse = inverse_finder_mod(divisor, downstairs_modulus_y)
    
    # 6 is the inverse of 2 in mod 11 becasue 6 * 2 = 1 (mod 11)
    # 6 is the inverse of 2 modulo 11, so multiply 6 to each side; y will have coefficient one.
    print(f'{inverse} is the inverse of {x_power} modulo {downstairs_modulus_y}, so multiply {inverse} to each side; y will have coefficient one.')
    ...
    # power == 6 % 11 == 6
    power %= modulus_y
    # Inverted-the-divisor equality: 1y = 6 (mod 11)
    print(f'Inverted-the-divisor equality: y = {power} (mod {downstairs_modulus_y})')
    ...
    print(f'Final solutions: x = {xs_mod_strings} (mod {modulus_x})')
